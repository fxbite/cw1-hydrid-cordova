...
function getLocation() {
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            showOnMap = `https://maps.google.com/?q=${position.coords.latitude},${position.coords.longitude}`
        })
    } else {
        console.log('Not support geoloction')
    }
}
getLocation()
...
$(document).on('click', '#btn-reset', function () {
    db.transaction(function (tx) {
        ...
        tx.executeSql(query, [], function (tx, result) {
            ...
            toast('Reset all data successfully.')
        }, transactionError);
    });
});
...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #fax').text($('#page-create #frm-register #fax').val());
...
function registerTrip(e) {
    ...
    let fax = $('#page-create #frm-register #fax').val()
    let country = $('#page-create #frm-register #country').val()

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Risk, Fax, Country, Map, Date) VALUES (?, ?, ?, ?, ?, ?, ?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description, risk, fax, country, showOnMap], transactionSuccess, transactionError);

        function transactionSuccess(tx, result) {
            let message = `Added trip '${name}' successfully.`;
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #fax').text(result.rows[0].Fax);
                $('#page-detail #detail #country').text(result.rows[0].Country);
                $('#page-detail #detail #map').attr("href", result.rows[0].Map);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #fax`).val(result.rows[0].Fax);
...
function updateTrip(e) {
    ...
    let fax = $('#page-detail #frm-update #fax').val();
    let country = $('#page-detail #frm-update #country').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Fax = ?, Country = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, risk, fax, country, id], transactionSuccess, transactionError);

        function transactionSuccess(tx, result) {
            let message = `Updated trip '${name}' successfully.`;
